...
def add_campaign_member(
    ...
    target_user_id: int, 
    role: str
) -> MemberResponse:  # Đổi kiểu trả về thành MemberResponse để chuẩn hóa
    require_campaign_owner(db, campaign_id, current_user_id)
    
    # 1. Kiểm tra user cần thêm có tồn tại không
    ...
    # 3. Tạo member mới và lưu vào CSDL
    ...
    # 4. Trả về MemberResponse chứa đầy đủ thông tin user để không bị null
    return MemberResponse(
        campaign_id=new_member.campaign_id,
